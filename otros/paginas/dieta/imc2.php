...
<?php
      ...
      $query1 = "select p_ideal from diagnostico";
      $resultados = $mysqli->query($query1);
	   $query2 = "select peso_actual from datos";
      $resultados2 = $mysqli->query($query2);
	  $rdp2=$resultados2->fetch_assoc();
	  $enpeso=0;
	  $nopeso=0;
	  
			while( $rdp=$resultados->fetch_assoc() )
    {	
				 if($rdp['p_ideal']=$rdp2['peso_acual'])
			  {
				  $enpeso += 1;
			  }  
			  else
	  ...
	  include ("conf/config.php");
      $query1 = "select diagnostico.id_persona, complexion from diagnostico, datos;";
      $resultados = $mysqli->query($query1);
	  $rdp2=$resultados2->fetch_assoc();
	  $enpeso=0;
	  $nopeso=0;
...
var pieData = [
			...
			{
			    value: <?php echo $enpeso ?>,
			    color: "#D97041"
			},
			{
			    value: <?php echo $nopeso ?>,
			    color: "#C7604C"
			}
            ];
